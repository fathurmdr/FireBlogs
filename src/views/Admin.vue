...
export default {
  ...
  methods: {
    async addAdmin() {
      ...
      const userRefByEmail = await getDocs(q);

      let userUid;
      userRefByEmail.forEach((doc) => {
        userUid = doc.id;
      });
      ...
      if (userUid) {
        ...
        this.functionMsg = `Success! ${this.adminEmail} has been made an admin!!`;
      } else {
        this.functionMsg = `Failed add ${this.adminEmail} as an admin!`;
      }
    },
  },
};
